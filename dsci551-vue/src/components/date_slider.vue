<template>
  <div>
      <b-col ref="dateSlider" id="date_slider"></b-col>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createSlider() {
        const that = this;
        const date_slider = noUiSlider.create(this.$refs.dateSlider, {
        ...
        date_slider.on("update", function(values, handle) {
            this[handle ? 'minDate' : 'maxDate'] = values[handle];
            that.minDate = this.get()[0];
            that.maxDate = this.get()[1];
            bus.$emit("date_update", [that.minDate, that.maxDate]);
            let dateValues = [
